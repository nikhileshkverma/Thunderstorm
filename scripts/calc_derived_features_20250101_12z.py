...
"""
================================================================================
DERIVED FEATURES CALCULATION  —  v1
================================================================================
...
PURPOSE
-------
Step 1: Extract HRRR features for cycle 12 UTC 1 January 2025,
        forecast hours 6, 12, 18, 21, 24, 30, 36
        at lat=28.51052, lon=-97.5052
...
------
  ...
  (7 rows × [424 extracted + 303 derived] columns)
...
DAY            = 1
CYCLE          = "12"
FORECAST_HOURS = [6, 12, 18, 21, 24, 30, 36]

LAT = 28.51052
LON = -97.5052

HRRR_BASE   = Path("/Storage03/nverma1/HRRR/2025")
OUTPUT_FILE  = "DerivedFeatures_20250101_12Z.csv"
...
def calc_diff_LFC_PSFC(row: dict) -> float:
    ...
    try:
        # Build pressure array [hPa], high→low (surface first = highest pressure)
        pres = [row["PRES_SFC"] / 100.0]
        tmp  = [row["TMP_2m"] - 273.15]
        # DPT_2m is extracted in K, convert to °C
        dpt  = [row["DPT_2m"] - 273.15]

        for lvl in range(1, 51):
            pres.append(row[f"PRES_{lvl}"] / 100.0)
            tmp.append(row[f"TMP_{lvl}"] - 273.15)
            # DPT at hybrid levels is derived (calc'd below) — get from row
            dpt.append(row.get(f"DPT_{lvl}", float("nan")))

        # Sort from high to low pressure (surface first)
        ...
        valid = [(p, t, d) for p, t, d in zip(pres_s, tmp_s, dpt_s)
                 if not (math.isnan(p) or math.isnan(t) or math.isnan(d))]
        if len(valid) < 3:
            return float("nan")
        ...
        p_arr  = pres_s * munits.hPa
        t_arr  = tmp_s  * munits.degC
        dp_arr = dpt_s  * munits.degC

        lfc_p, _ = mpcalc.lfc(p_arr, t_arr, dp_arr, which='top')
        lfc_hPa  = lfc_p.to(munits.hPa).magnitude
        psfc_hPa = row["PRES_SFC"] / 100.0
        return float(lfc_hPa - psfc_hPa)
...
def compute_derived(row: dict) -> dict:
    ...
    SHTFL = row["SHTFL"]
    LHTFL = row["LHTFL"]
    row["BowenRatio_Mean"] = calc_BowenRatio(SHTFL, LHTFL) if (SHTFL and LHTFL) else float("nan")
...
def spot_check_derived(df: pd.DataFrame):
    ...
    checks = [
        # POT at level 1: θ = T*(P_SFC/P)^0.2854
        ("POT_1",         r.get("POT_1"),
        ...
        # VTMPLR for layer 1-2
        ("VTMPLR_12",     r.get("VTMPLR_12"),     "Virtual Temp Lapse Rate lyr 1-2 [10-3 K/m]"),
        # VS for layer 1-2
        ("VS_12",         r.get("VS_12"),          "Vert Wind Shear lyr 1-2 [10-3 s-1]"),
        # Bowen Ratio
        ("BowenRatio_Mean", r.get("BowenRatio_Mean"), "SHTFL/LHTFL"),
        # ZL
        ("ZL_MIN",        r.get("ZL_MIN"),         "-Z/L stability"),
        ("ZL_MAX",        r.get("ZL_MAX"),         "-Z/L stability (same as ZL_MIN, single pt)"),
        # diff_LFC_PSFC
        ("diff_LFC_PSFC", r.get("diff_LFC_PSFC"), "LFC - PSFC [hPa]"),
    ]
    ...
    for col in single_val:
        if col in df.columns:
            ...
            if n > 0:
                ...
                print(f"  ✗ {col:<15s} NaN in {n}/7 rows (FH: {fhs})")
                has_null = True
    if not has_null:
